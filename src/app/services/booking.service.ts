...
export class BookingService {
  ...
  getRestaurantTables(id:number) {
    return this.http.get<TableDTO[]>(`https://localhost:5001/api/tables/restaurant/${id}`, { headers: this.headers });
  }

  getReservationsForTable(id:number, date: Date) {

    return this.http.get<ReservationDTO[]>(`https://localhost:5001/api/tables/${id}/reservations`, {
        params: {
